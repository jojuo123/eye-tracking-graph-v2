...
to match whatever units the distance map is in. This script visualizes all three for a few
samples per split: the normalized-space matrix, the raw pixel-space matrix at the same `tau`,
and a pixel-space matrix whose `tau` is auto-rescaled by the empirical ratio between the two
distance maps (so its spread is comparable to the normalized-space one).
